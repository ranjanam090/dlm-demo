...
function init() {
  for (let i=0; i<STALL_COUNT; i++) {
    state.stalls.push({ id: i+1, connected: false, requestKw: 0, allocKw: 0, priority: 3 });
  }
  renderStalls();
  renderPoolModules();
  updateSummary();
...
function renderPoolModules() {
  ...
  for (let i=0; i<total; i++){
    ...
    if (i >= free) { mod.style.opacity = .25; mod.style.filter = 'grayscale(1)'; }
...
/** Allocation (50 kW steps, priority-aware, greedy round-robin) **/
function allocate() {
...
function allocateAndAnimate() {
  ...
  state.stalls.forEach((s, idx) => {
    ...
    const pct = (after / (MAX_SITE_KW / 2)) * 100; // visual scale
...
function flyToken(pointA, pointB, reverse=false){
  ...
  const cp = { x: (pointA.x + pointB.x)/2 + (reverse ? -60 : 60), y: Math.min(pointA.y, pointB.y) - 60 };
  ...
  function step(ts){
    ...
    if (p < 1 && document.body.dataset.paused !== 'true') requestAnimationFrame(step);
    else { token.classList.add('fade'); setTimeout(()=> token.remove(), 160); }
  }
...
function flyFromPoolToStall(stallId){
  ...
  flyToken(getElemCenter(poolRef), getElemCenter(toRef), false);
}
...
function flyFromStallToPool(stallId){
  ...
  flyToken(getElemCenter(fromRef), getElemCenter(poolRef), true);
}
